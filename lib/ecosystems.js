...
export async function * fetchEcosystemDependents (name, options = {}) {
  ...
  const itemsPerPage = perPage || 36;

  /**
   * Wrapper to track page numbers as items are yielded
   *
   * @param {AsyncGenerator<unknown, void, undefined>} source
   */
  async function * withPageTracking (source) {
    let currentPage = 1;

    for await (const item of source) {
      // Estimate page based on items seen
      currentPage = Math.floor(itemsProcessed / itemsPerPage) + 1;

      yield { item, page: currentPage };
    }
  }

  // Process a couple of items at once
  yield * bufferedAsyncMap(
    withPageTracking(fetchJsonPages(client, `https://packages.ecosyste.ms/api/v1/registries/npmjs.org/packages/${name}/dependent_packages${includeHistoricDependents ? '' : '?latest=true'}`, {
      maxPages,
      perPage,
    })),
    async function * ({ item, page }) {
      ...
      // Skip packages that are in the skipList
      if (skipList?.has(dependent)) {
        logger && logger.debug(`Skipping "${dependent}", already in skipList`);
        return;
      }
